...
/**
 * hrms/transaction/claim/apis/claimdt-open.php
 *
 * ==========
 * Detil-Open
 * ==========
...
$API = new class extends claimBase {
	...
	public function execute($options) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new claim_claimdtHandler($options);
			$hnd->caller = &$this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$result->record = array_merge($record, [
				'claimdt_dt' => date("d/m/Y", strtotime($record['claimdt_dt'])),
...
/*{__LOOKUPUSERMERGE__}*/
			...
			try { $result->record['claimdt_file_doc'] = $this->cdb->getAttachment($file_id, 'filedata'); } catch (\Exception $ex) {}
