...
import * as hnd from  './claim-claimdtgrid-hnd.mjs'

const tbl_list = $('#pnl_editclaimdtgrid-tbl_list');
const txt_title = $('#pnl_editclaimdtgrid-title');
const pnl_control = $('.pnl_editclaimdtgrid-control');
const btn_removechecked  = $('#pnl_editclaimdtgrid-removechecked');
const btn_addrow = $('#pnl_editclaimdtgrid-addrow');
...
export function createnew(data, options) {
	...
	txt_title.html(data.claimdt_dt)
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/claimdt-list`
		options.criteria['id'] = data.claim_id
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editclaimdtform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editclaimdtgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editclaimdtgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editclaimdtform', () => {
		$ui.getPages().ITEMS['pnl_editclaimdtform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/claimdt-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editclaimdtform', ()=>{
		$ui.getPages().ITEMS['pnl_editclaimdtform'].handler.createnew(header_data)
	})	
}