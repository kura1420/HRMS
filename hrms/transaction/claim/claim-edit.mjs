...
import * as hnd from  './claim-edit-hnd.mjs'
...
const obj = {
	txt_claim_id: $('#pnl_edit-txt_claim_id'),
	txt_claim_code: $('#pnl_edit-txt_claim_code'),
	...
	txt_claim_requestby: $('#pnl_edit-txt_claim_requestby'),
	txt_claim_requestdate: $('#pnl_edit-txt_claim_requestdate'),
...
export async function init(opt) {
	...
	new fgta4slideselect(obj.cbo_activity_id, {
		...
		OnDataLoading: (criteria, options) => {
			...
			if (typeof hnd.cbo_activity_id_dataloading === 'function') {
				hnd.cbo_activity_id_dataloading(criteria, options);
			}						
		},					
		OnDataLoaded : (result, options) => {
			
			if (typeof hnd.cbo_activity_id_dataloaded === 'function') {
				hnd.cbo_activity_id_dataloaded(result, options);
			}
		},
		OnSelected: (value, display, record, args) => {
			if (value!=args.PreviousValue ) {
				if (typeof hnd.cbo_activity_id_selected === 'function') {
					hnd.cbo_activity_id_selected(value, display, record, args);
				}
			}
		},
...
export function createnew() {
	...
	form.createnew(async (data, options)=>{
		...
		$ui.getPages().ITEMS['pnl_editclaimdtgrid'].handler.createnew(data, options)
...
function btn_print_click() {
	...
	var args = {
		...
		reportmodule: `${module}/claim.xprint?renderto=${renderto}&format=${format}`,
		handled: false
	}
