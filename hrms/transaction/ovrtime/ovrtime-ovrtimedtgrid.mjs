...
import * as hnd from  './ovrtime-ovrtimedtgrid-hnd.mjs'

const tbl_list = $('#pnl_editovrtimedtgrid-tbl_list');
const txt_title = $('#pnl_editovrtimedtgrid-title');
const pnl_control = $('.pnl_editovrtimedtgrid-control');
const btn_removechecked  = $('#pnl_editovrtimedtgrid-removechecked');
const btn_addrow = $('#pnl_editovrtimedtgrid-addrow');
...
export function createnew(data, options) {
	...
	txt_title.html(data.ovrtimedt_dt)
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/ovrtimedt-list`
		options.criteria['id'] = data.ovrtime_id
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editovrtimedtform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editovrtimedtgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editovrtimedtgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editovrtimedtform', () => {
		$ui.getPages().ITEMS['pnl_editovrtimedtform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/ovrtimedt-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editovrtimedtform', ()=>{
		$ui.getPages().ITEMS['pnl_editovrtimedtform'].handler.createnew(header_data)
	})	
}