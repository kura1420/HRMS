...
if (is_file(__DIR__ .'/data-ovrtimedt-handler.php')) {
	require_once __DIR__ .'/data-ovrtimedt-handler.php';
}
...
/**
 * hrms/transaction/ovrtime/apis/ovrtimedt-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel ovrtimedt ovrtime (trn_ovrtimedt)
 *
...
$API = new class extends ovrtimeBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'trn_ovrtimedt';
		$primarykey = 'ovrtimedt_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\ovrtime_ovrtimedtHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new ovrtime_ovrtimedtHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'ovrtimedt_id', 'ovrtime_id', 'ovrtimedt_dt', 'ovrtime_id_type',
					'ovrtimedt_descr'
			];
			...
			$obj->ovrtimedt_dt = (\DateTime::createFromFormat('d/m/Y',$obj->ovrtimedt_dt))->format('Y-m-d');



			if ($obj->ovrtimedt_descr=='') { $obj->ovrtimedt_descr = '--NULL--'; }
			...
			try {
				...
				$header_table = 'trn_ovrtime';
				$header_primarykey = 'ovrtime_id';
				$detil_primarykey = 'ovrtime_id';
				...
				// result
				$options->criteria = [
					"ovrtimedt_id" => $obj->ovrtimedt_id
				];

				$criteriaValues = [
					"ovrtimedt_id" => " ovrtimedt_id = :ovrtimedt_id "
				];
				...
				$sqlFieldList = [
					'ovrtimedt_id' => 'A.`ovrtimedt_id`', 'ovrtime_id' => 'A.`ovrtime_id`', 'ovrtimedt_dt' => 'A.`ovrtimedt_dt`', 'ovrtime_id_type' => 'A.`ovrtime_id_type`',
					'ovrtimedt_descr' => 'A.`ovrtimedt_descr`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "trn_ovrtimedt A";
				$sqlWhere = $where->sql;
				...
				$dataresponse = array_merge($record, [
					...
					'ovrtimedt_dt' => date("d/m/Y", strtotime($row['ovrtimedt_dt'])),
					'ovrtime_name' => \FGTA4\utils\SqlUtility::Lookup($record['ovrtime_id_type'], $this->db, 'mst_ovrtime', 'ovrtime_id', 'ovrtime_name'),
