...
class ovrtime_headerHandler extends WebAPI  {

	public function buildListCriteriaValues(object &$options, array &$criteriaValues) : void
	{
		$criteriaValues['empl_id'] = " A.empl_id = :empl_id "; 
		$criteriaValues['ra_empl_id'] = " A.empl_id = :ra_empl_id OR (A.empl_id != :ra_empl_id AND A.ovrtime_isrequest = 1)"; 
	}

	public function PreCheckInsert($data, $obj, $options) {
		try {
			$obj->ovrtime_code = $this->generateCode($obj->docapprv_id);
		} catch (\Throwable $th) {
			throw $th;
		}
	}
	...
	protected function generateCode($docapprv_id) {
		$sql = "SELECT ovrtime_code FROM trn_ovrtime ORDER BY _createdate DESC LIMIT 1";
		$res = $this->db->query($sql);
		$ovrtime_code = $res->fetchColumn();

		if ($ovrtime_code) {
			$split = explode('/', $ovrtime_code);
