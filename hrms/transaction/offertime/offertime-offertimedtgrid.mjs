...
import * as hnd from  './offertime-offertimedtgrid-hnd.mjs'

const tbl_list = $('#pnl_editoffertimedtgrid-tbl_list');
const txt_title = $('#pnl_editoffertimedtgrid-title');
const pnl_control = $('.pnl_editoffertimedtgrid-control');
const btn_removechecked  = $('#pnl_editoffertimedtgrid-removechecked');
const btn_addrow = $('#pnl_editoffertimedtgrid-addrow');
...
export function createnew(data, options) {
	...
	txt_title.html(data.offertimedt_dt)
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/offertimedt-list`
		options.criteria['id'] = data.offertime_id
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editoffertimedtform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editoffertimedtgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editoffertimedtgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editoffertimedtform', () => {
		$ui.getPages().ITEMS['pnl_editoffertimedtform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/offertimedt-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editoffertimedtform', ()=>{
		$ui.getPages().ITEMS['pnl_editoffertimedtform'].handler.createnew(header_data)
	})	
}