...
export function init(ed) {
	...
	form.setDisable(obj.txt_offertime_code, true);
...
{
	data.offertime_code = '[AUTO]';

	data.docapprv_id = 'OFTR';
	data.docapprv_name = 'Offertime';
...
function btn_request_click() 
{
	...
	if (caption.includes('View')) {
		let args = {
			data: {
				offertime_id: form.getValue(obj.txt_offertime_id),
...
async function posting(args) 
{
	try {
		...
		form.setValue(obj.chk_offertime_isrequest, dataresponse.offertime_isrequest);
		form.setValue(obj.chk_offertime_isapproved, dataresponse.offertime_isapproved);
		form.setValue(obj.chk_offertime_isdeclined, dataresponse.offertime_isdeclined);
		form.setValue(obj.txt_offertime_rejectnotes, dataresponse.offertime_rejectnotes);
...
function setButtonRAD(record)
{
	...
	if (opt.variancedata.is_request_myself == true && opt.variancedata.can_approve == false) {
		if (record.offertime_isrequest == 0) {
		...
		if (record.offertime_isrequest == 1) {
		...
		if (record.offertime_isapproved == 1 || record.offertime_isdeclined == 1) {
			btn_request_on = false;
			btn_unrequest_on = false;
			btn_approve_on = false;
			btn_decline_on = false;
