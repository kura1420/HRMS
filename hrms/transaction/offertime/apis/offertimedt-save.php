...
if (is_file(__DIR__ .'/data-offertimedt-handler.php')) {
	require_once __DIR__ .'/data-offertimedt-handler.php';
}
...
/**
 * hrms/transaction/offertime/apis/offertimedt-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel offertimedt offertime (trn_offertimedt)
 *
...
$API = new class extends offertimeBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'trn_offertimedt';
		$primarykey = 'offertimedt_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\offertime_offertimedtHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new offertime_offertimedtHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'offertimedt_id', 'offertime_id', 'offertimedt_dt', 'offertime_id_type',
					'offertimedt_descr'
			];
			...
			$obj->offertimedt_dt = (\DateTime::createFromFormat('d/m/Y',$obj->offertimedt_dt))->format('Y-m-d');



			if ($obj->offertimedt_descr=='') { $obj->offertimedt_descr = '--NULL--'; }
			...
			try {
				...
				$header_table = 'trn_offertime';
				$header_primarykey = 'offertime_id';
				$detil_primarykey = 'offertime_id';
				...
				// result
				$options->criteria = [
					"offertimedt_id" => $obj->offertimedt_id
				];

				$criteriaValues = [
					"offertimedt_id" => " offertimedt_id = :offertimedt_id "
				];
				...
				$sqlFieldList = [
					'offertimedt_id' => 'A.`offertimedt_id`', 'offertime_id' => 'A.`offertime_id`', 'offertimedt_dt' => 'A.`offertimedt_dt`', 'offertime_id_type' => 'A.`offertime_id_type`',
					'offertimedt_descr' => 'A.`offertimedt_descr`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "trn_offertimedt A";
				$sqlWhere = $where->sql;
				...
				$dataresponse = array_merge($record, [
					...
					'offertimedt_dt' => date("d/m/Y", strtotime($row['offertimedt_dt'])),
					'offertime_name' => \FGTA4\utils\SqlUtility::Lookup($record['offertime_id_type'], $this->db, 'mst_offertime', 'offertime_id', 'offertime_name'),
