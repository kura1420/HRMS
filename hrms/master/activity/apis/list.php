...
/**
 * hrms/master/activity/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header activity (mst_activity)
 ...
 * tanggal 23/05/2024
 */
$API = new class extends activityBase {

	public function execute($options) {

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\activity_headerHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new activity_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$criteriaValues = [
				"search" => " A.activity_id LIKE CONCAT('%', :search, '%') "
			];
			...
			$sqlFieldList = [
				'activity_id' => 'A.`activity_id`', 'activity_name' => 'A.`activity_name`', 'activity_descr' => 'A.`activity_descr`', 'activity_isdisabled' => 'A.`activity_isdisabled`',
				'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
			];
			$sqlFromTable = "mst_activity A";
			$sqlWhere = $where->sql;
