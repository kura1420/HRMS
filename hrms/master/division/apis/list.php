...
/**
 * hrms/master/division/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header division (mst_division)
...
$API = new class extends divisionBase {
	...
	public function execute($options) {
		...
		$handlerclassname = "\\FGTA4\\apis\\division_headerHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new division_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$criteriaValues = [
				"search" => " A.division_code LIKE CONCAT('%', :search, '%') OR A.division_name LIKE CONCAT('%', :search, '%') "
			];
