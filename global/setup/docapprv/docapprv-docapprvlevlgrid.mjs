...
import * as hnd from  './docapprv-docapprvlevlgrid-hnd.mjs'

const tbl_list = $('#pnl_editdocapprvlevlgrid-tbl_list');
const txt_title = $('#pnl_editdocapprvlevlgrid-title');
const pnl_control = $('.pnl_editdocapprvlevlgrid-control');
const btn_removechecked  = $('#pnl_editdocapprvlevlgrid-removechecked');
const btn_addrow = $('#pnl_editdocapprvlevlgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/docapprvlevl-list`
		options.criteria['id'] = data.docapprv_id
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editdocapprvlevlform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editdocapprvlevlgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editdocapprvlevlgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editdocapprvlevlform', () => {
		$ui.getPages().ITEMS['pnl_editdocapprvlevlform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/docapprvlevl-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editdocapprvlevlform', ()=>{
		$ui.getPages().ITEMS['pnl_editdocapprvlevlform'].handler.createnew(header_data)
	})	
}