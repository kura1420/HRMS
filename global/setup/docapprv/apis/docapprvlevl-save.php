...
if (is_file(__DIR__ .'/data-docapprvlevl-handler.php')) {
	require_once __DIR__ .'/data-docapprvlevl-handler.php';
}
...
/**
 * global/setup/docapprv/apis/docapprvlevl-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel docapprvlevl docapprv (mst_docapprvlevl)
 *
...
$API = new class extends docapprvBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_docapprvlevl';
		$primarykey = 'docapprvlevl_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\docapprv_docapprvlevlHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new docapprv_docapprvlevlHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->docapprvlevl_id = strtoupper($obj->docapprvlevl_id);
			$obj->docapprv_id = strtoupper($obj->docapprv_id);
			...
			try {
				...
				$header_table = 'mst_docapprv';
				$header_primarykey = 'docapprv_id';
				$detil_primarykey = 'docapprv_id';
				...
				// result
				$options->criteria = [
					"docapprvlevl_id" => $obj->docapprvlevl_id
				];

				$criteriaValues = [
					"docapprvlevl_id" => " docapprvlevl_id = :docapprvlevl_id "
				];
