...
/**
 * global/setup/docapprv/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header docapprv (mst_docapprv)
 *
...
$API = new class extends docapprvBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_docapprv';
		$primarykey = 'docapprv_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\docapprv_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new docapprv_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			if ($obj->docapprv_descr=='') { $obj->docapprv_descr = '--NULL--'; }
			...
			try {
				...
				$criteriaValues = [
					"docapprv_id" => " docapprv_id = :docapprv_id "
				];
				...
				$sqlFieldList = [
					'docapprv_id' => 'A.`docapprv_id`', 'docapprv_name' => 'A.`docapprv_name`', 'docapprv_descr' => 'A.`docapprv_descr`', 'docapprv_isdisabled' => 'A.`docapprv_isdisabled`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_docapprv A";
				$sqlWhere = $where->sql;
