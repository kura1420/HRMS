...
import * as hnd from  './docapprv-docapprvlevlform-hnd.mjs'

const reload_header_modified = true;

const txt_caption = $('#pnl_editdocapprvlevlform-caption')
const txt_title = $('#pnl_editdocapprvlevlform-title')
const btn_edit = $('#pnl_editdocapprvlevlform-btn_edit')
const btn_save = $('#pnl_editdocapprvlevlform-btn_save')
const btn_delete = $('#pnl_editdocapprvlevlform-btn_delete')
const btn_prev = $('#pnl_editdocapprvlevlform-btn_prev')
const btn_next = $('#pnl_editdocapprvlevlform-btn_next')
const btn_addnew = $('#pnl_editdocapprvlevlform-btn_addnew')
const chk_autoadd = $('#pnl_editdocapprvlevlform-autoadd')


const pnl_form = $('#pnl_editdocapprvlevlform-form')
const obj = {
...
export async function init(opt) {
	...
	form = new global.fgta4form(pnl_form, {
		primary: obj.txt_docapprvlevl_id,
		autoid: true,
		logview: 'mst_docapprvlevl',
		btn_edit: btn_edit,
		btn_save: btn_save,
	...
	obj.cbo_empl_id.name = 'pnl_editdocapprvlevlform-cbo_empl_id'		
	...
	document.addEventListener('OnButtonBack', (ev) => {
		...
		if ($ui.getPages().getCurrentPage()==this_page_id) {
			...
			if (form.isDataChanged()) {
				form.canceledit(()=>{
					$ui.getPages().show('pnl_editdocapprvlevlgrid', ()=>{
						form.setViewMode()
						$ui.getPages().ITEMS['pnl_editdocapprvlevlgrid'].handler.scrolllast()
					})					
				})
			} else {
				$ui.getPages().show('pnl_editdocapprvlevlgrid', ()=>{
...
export function open(data, rowid, hdata) {
	...
	txt_title.html(header_data.empl_id)
	if (typeof hnd!=='undefined') { 
	...
	var fn_dataopening = async (options) => {
		options.api = `${global.modulefullname}/docapprvlevl-open`
...
export function createnew(hdata) {
	...
	form.createnew(async (data, options)=>{
		data.docapprv_id = hdata.docapprv_id
		data.docapprvlevl_value = 0

		data.docapprvlevl_sortorder = 0
...
async function form_datasaving(data, options) {
	options.api = `${global.modulefullname}/docapprvlevl-save`
...
async function form_datasaved(result, options) {
	...
	*/
	...
	form.rowid = $ui.getPages().ITEMS['pnl_editdocapprvlevlgrid'].handler.updategrid(data, form.rowid)
	var rowdata = {
		data: data,
		rowid: form.rowid
	}
...
async function form_deleting(data, options) {
	options.api = `${global.modulefullname}/docapprvlevl-delete`
...
async function form_deleted(result, options) {
	...
	$ui.getPages().show('pnl_editdocapprvlevlgrid', ()=>{
		$ui.getPages().ITEMS['pnl_editdocapprvlevlgrid'].handler.removerow(form.rowid)
	});
...
function form_idsetup(options) {
	var objid = obj.txt_docapprvlevl_id
...
function btn_prev_click() {
	...
	var record = $ui.getPages().ITEMS['pnl_editdocapprvlevlgrid'].handler.getGrid().DATA[dataid]
