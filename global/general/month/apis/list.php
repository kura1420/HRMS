...
/**
 * global/general/month/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header month (mst_month)
 ...
 * tanggal 07/06/2024
 */
$API = new class extends monthBase {

	public function execute($options) {

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\month_headerHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new month_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$criteriaValues = [
				"search" => " A.month_id LIKE CONCAT('%', :search, '%') OR A.month_name LIKE CONCAT('%', :search, '%') "
			];
			...
			$sqlFieldList = [
				'month_id' => 'A.`month_id`', 'month_name' => 'A.`month_name`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
				'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
			];
			$sqlFromTable = "mst_month A";
			$sqlWhere = $where->sql;
