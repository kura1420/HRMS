...
import * as hnd from  './reqstion-reqstionitmgrid-hnd.mjs'

const tbl_list = $('#pnl_editreqstionitmgrid-tbl_list');
const txt_title = $('#pnl_editreqstionitmgrid-title');
const pnl_control = $('.pnl_editreqstionitmgrid-control');
const btn_removechecked  = $('#pnl_editreqstionitmgrid-removechecked');
const btn_addrow = $('#pnl_editreqstionitmgrid-addrow');
...
export function createnew(data, options) {
	...
	txt_title.html(data.reqstionitm_dt)
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/reqstionitm-list`
		options.criteria['id'] = data.reqstion_id
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editreqstionitmform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editreqstionitmgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editreqstionitmgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editreqstionitmform', () => {
		$ui.getPages().ITEMS['pnl_editreqstionitmform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/reqstionitm-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editreqstionitmform', ()=>{
		$ui.getPages().ITEMS['pnl_editreqstionitmform'].handler.createnew(header_data)
	})	
}