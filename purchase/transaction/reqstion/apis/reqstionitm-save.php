...
/**
 * purchase/transaction/reqstion/apis/reqstionitm-save.php
 *
 * ==========
 * Detil-Save
 * ==========
...
$API = new class extends reqstionBase {
	...
	public function execute($data, $options, $files) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new reqstion_reqstionitmHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'reqstionitm_id', 'typereq_id', 'reqstionitm_name', 'reqstionitm_val',
					'reqstionitm_file', 'reqstionitm_descr', 'reqstion_id'
			];
			...
			if ($obj->reqstionitm_file=='') { $obj->reqstionitm_file = '--NULL--'; }
			if ($obj->reqstionitm_descr=='') { $obj->reqstionitm_descr = '--NULL--'; }
