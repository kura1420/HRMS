...
/**
 * purchase/transaction/reqstion/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header reqstion (trn_reqstion)
...
$API = new class extends reqstionBase {
	...
	public function execute($options) {
		...
		try {
			...
			$criteriaValues = [
				"search" => " A.reqstion_code LIKE CONCAT('%', :search, '%') OR A.reqstion_name LIKE CONCAT('%', :search, '%') "
			];
			...
			foreach ($rows as $row) {
				...
				*/
				...
				$this->addFields('reqstion_requestby', 'reqstion_requestby', $record, $GLOBALS['MAIN_USERTABLE'], 'user_fullname', 'user_id');
				$this->addFields('reqstion_approveby', 'reqstion_approveby', $record, $GLOBALS['MAIN_USERTABLE'], 'user_fullname', 'user_id');
				$this->addFields('reqstion_declineby', 'reqstion_declineby', $record, $GLOBALS['MAIN_USERTABLE'], 'user_fullname', 'user_id');
				$this->addFields('reqstion_executeby', 'reqstion_executeby', $record, $GLOBALS['MAIN_USERTABLE'], 'user_fullname', 'user_id');
