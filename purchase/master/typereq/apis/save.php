...
/**
 * purchase/master/typereq/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header typereq (mst_typereq)
 *
 ...
 * tanggal 15/06/2024
 */
$API = new class extends typereqBase {
	
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_typereq';
		$primarykey = 'typereq_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\typereq_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new typereq_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				// result
				$options->criteria = [
					"typereq_id" => $obj->typereq_id
				];

				$criteriaValues = [
					"typereq_id" => " typereq_id = :typereq_id "
				];
				...
				$sqlFieldList = [
					'typereq_id' => 'A.`typereq_id`', 'typereq_name' => 'A.`typereq_name`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_typereq A";
				$sqlWhere = $where->sql;
